...
test("renders localised legal documents with public-facing metadata", async ({ page }) => {
  ...
  await expect(page.locator("article.legal-copy")).toContainText("conservate principalmente in locale");
  await expect(page.locator("article.legal-copy")).toContainText("acquisto in-app PRO opzionale");
  ...
  await expect(page.locator("article.legal-copy")).toContainText("primarily stored locally on your device");
